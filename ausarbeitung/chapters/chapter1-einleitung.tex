...
\chapter{Introduction}
	...
	The author assumes that the approach can be used on consecutive frames, but one problem remains when using only line detection: during later phases of the game many stones will be placed especially in the center of the image. The lines are occluded by tokens and there's no way left to detect the board.
	\\
	...
	There are several patents which protect apparatuses for the detection of Go games mostly from Japanese inventors. Most of them rely on some hardware which detects where a piece has been placed. There are numerous approaches in this direction, for example using magnetic pieces \cite{JPH11114129A} or pressure detection \cite{JP2002000792A}.
