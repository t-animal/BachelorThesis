...
\chapter{Detecting the board}
	\label{detector}
	\todo[inline]{Nochmal gegen tatsaechlichen Detector pruefen, ob alles gleich ist}
	...
	The implementation\todo{Hier korrekte Github URL einfuegen und dort veroeffentlichen}\footnote{The source code is GPLv3 licensed and can be freely accessed at https://github.com/t-animal/} is based on the OpenCV framework (version 2.4.10). If not noted otherwise all detections have been performed on an x86\_64 architecture. As described in chapter 3 this does not give different results than the execution on mobile devices (i.e. ARM architecture), for example because of differences in floating point calculations.
